...
def parsePackets(ifile, packets):
    ...
    while next_byte:
        ...
        total_bytes_read += 1
        possible_tail = struct.unpack(
            HamsterPacket.FORMAT_TAIL, tail_search_fifo[-HamsterPacket.LEN_TAIL :]
        )

        # if the fifo now contains a tail
        if (
            possible_tail[0] == HamsterPacket.MAGIC1
            and possible_tail[1] == HamsterPacket.MAGIC2
            and possible_tail[-1] == HamsterPacket.MAGIC3
        ):
            ...
            try:
                ...
                print(e)
            cur_bytes_discarded = len(tail_search_fifo) - (
                packet_len if packet_len else 0
            )
            if cur_bytes_discarded > 0:
                print(
                    cur_bytes_discarded,
                    "bytes discared. Packet #",
        ...
        if (
            len(tail_search_fifo) > HamsterPacket.MAX_ERROR_PACKET_LEN
            and not HamsterPacket.IGNORE_MAX_ERROR
        ):
            print(
                "Error: exceeded max packet length in search of tail (offset 0x{:X}).".format(
                    ifile.tell()
                )
            )
            # print(tail_search_fifo)
    ...
    print(
        ...
        "/",
        total_bytes_read,
        "bytes discarded in total due to errors or initial padding.",
    )
    return 0
...
def exportPackets(dirname, packets):
    ...
    for typeID in packets:
        with open(
            "{}/{}.csv".format(dirname, HamsterPacket.packet_name[typeID]),
            "w",
            newline="",
        ) as ofile:
            csvwriter = csv.writer(ofile)
            header = (
                ["Packet #", "Checksum Status", "Output Checksum"]
                + HamsterPacket.tail_fields
                + HamsterPacket.packet_fields[typeID]
                + ["Packet (hex)"]
            )
...
def makeTable(title, comment, report, index, padTo):
    ...
    for state, statePacketTypes in report.items():
        ...
        for sensorName in table.field_names[1:]:
            row[counter] = (
                padArray(statePacketTypes[sensorName][index], padTo)
                if sensorName in statePacketTypes
                else None
            )
            counter = counter + 1
...
def statePacketFrequency(packets):
    ...
    for tail in packets.tails:
        ...
        if (prevProcessedTail != None) and (
            tail[TAIL_STATE] != prevProcessedTail[TAIL_STATE]
        ):
        ...
        if curTailType not in prevTails:
            ...
            delta = (
                int(tail[TAIL_PWR_CTR]) - int(prevTails[curTailType][TAIL_PWR_CTR])
            ) / mSEC_TO_uSEC
    ...
    for state, statePacketTypes in deltas.items():
        for packetType, statePacketDeltas in statePacketTypes.items():
            ...
            if len(statePacketDeltas) < 2:
                ...
                binwidth = 2
                bins = bins = range(
                    int(min(statePacketDeltas)),
                    int(max(statePacketDeltas)) + binwidth,
                    binwidth,
                )
...
def main():
    parser = addargs(
        argparse.ArgumentParser(
            description="Parser for HAMSTER's FlashLog data. Generates 1 CSV file per packet type."
        )
    )
    ...
    with open(args.input_file, "rb") as ifile:
        if not HamsterHelpers.seekToDump(ifile, HamsterPacket.BINDUMP_OPEN_TAG):
            ...
            try:
                ...
                print(
                    "Stopped parsing at ox{:08x} due to parse error.".format(
                        ifile.tell()
                    )
                )
