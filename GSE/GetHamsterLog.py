...
def addargs(parser):
    parser.add_argument(
        "port", help="Device file or COM port to which the HAMSTER is attached"
    )
    parser.add_argument(
        "-o",
        "--output_file",
    ...
    parser.add_argument(
        "-d", "--dump-baud", help="Baud rate for receiving the actual dump data"
    )
    parser.add_argument(
        "-f",
        "--flight",
...
def main():
    parser = argparse.ArgumentParser(
        description="This program grabs a binary dump from the FlashLog. To use it,\
    ...
    with serial.Serial(args.port, args.baud, timeout=2) as ser, open(
        args.output_file, "wb"
    ) as ofile:
        bytes_received = 0
        time.sleep(2)
        try:
            if args.flight:
                print(
                    "Expecting to interact with the HAMSTER FSW, currently in state STANDBY."
                )
                print("Initiating hex dump...")
                ser.write(
                    "dump\n".encode("ascii")
                )  # dump command is valid from STANDBY state
            ...
            time.sleep(
                0.05
            )  # wait a few ms for the previous write to go through (can't find another way than this)
            ...
            while len(byte) > 0:
                ...
                if time.time() - last_print_time >= 1:
                    ...
                    # calc speed in kiB/s
                    speed = ((bytes_received - last_print_bytes) / 1024) / (
                    ...
                    print(
                        "\r{} bytes received, speed = {:.2f}kiB/s.\t".format(
                            bytes_received, speed
                        ),
                        end="",
                    )
                ofile.write(byte)
