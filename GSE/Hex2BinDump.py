...
def hexdump2Bindump(ifile, ofile, reverse=False, compressed=False):
    ...
    if compressed:
        byte_list = heatshrink2.decompress(
            byte_list, input_buffer_size=1024, window_sz2=10, lookahead_sz2=4
        )
        byte_list = bytearray(byte_list)
    ...
    while True:
        ...
        if cur == "\n" or cur == "\r":
            ...
            if len(frame_bytes) > 0:
                if compressed:
                    frame_bytes = heatshrink2.decompress(
...
def main():
    ...
    with open(args.input_file, "rb") as ifile, open(args.output_file, "wb") as ofile:
        ...
        elif HamsterHelpers.seekToDump(
            ifile, HamsterPacket.COMPRESSED_HEX_REV_OPEN_TAG
        ):
            print("Compressed Hex Reverse Dump found. Converting to bindump...")
            hexdump2Bindump(ifile, ofile, reverse=True, compressed=True)
        elif HamsterHelpers.seekToDump(
            ifile, HamsterPacket.COMPRESSED_BASE64_REV_OPEN_TAG
        ):
