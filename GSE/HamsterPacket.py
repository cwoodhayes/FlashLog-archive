...
REVERSE_CHECKSUM_OFFSET = 8 	#offset of the start of the checksum field fron the end of the packet tail
...
MAX_ERROR_PACKET_LEN = MAX_PACKET_LEN + LEN_TAIL - 1 	#text packet written except 1 byte, then LOG_INVALID tail tacked on
...
class HamsterPackets(dict):
	...
	def addPacket(self, packet_bytes, tail_struct):
		...
		if ((packet_type == 7) or (packet_type == 0)): #invalid packet handling
			...
			try:
				...
				data_struct = struct.unpack(packet_format[packet_type], cur_packet_bytes)
			...
			data_to_checksum = packet_bytes[-packet_len[packet_type]-LEN_TAIL:-REVERSE_CHECKSUM_OFFSET]
	...
	def reportContents(self):
		...
		for i in range(0, len(packet_name)):
			...
			print('\t{: <20}: {} ({} invalid) packets'.format(packet_name[i], len(self[i]), invalid_packets))
...
def print_tail(unpacked_struct_tail):
	print('[{:04X}-{:02X}]'.format(unpacked_struct_tail[0], unpacked_struct_tail[1]), end='')
